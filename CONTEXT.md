...
- **OCR run**: one recognition pass over the current page (`ocrPipeline.ocrPage`), producing `{ raw, markdown?, engine }` from the local Ollama model or the tesseract fallback. Runs land in the **OCR history** (max 3, newest first) and reopen in the review modal.
