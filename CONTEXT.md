...
- **OCR document**: one archived OCR result from the Tools OCR Scanner — a titled Markdown document (title auto-generated from content, user-editable) plus metadata (source filename, page count, engine, date), persisted locally in IndexedDB and reopened in the scanner's editor. _Avoid_: mixing it up with an OCR run (a single-page pass in the editor).
