...
export function estimateGlyphsForRun(text = "", width = 0, fontSize = 12) {
  ...
  const estimatedWidth = weights.reduce(
    (sum, weight) => sum + weight * fontSize,
    0,
  );
...
export function buildExtractedTextMetrics(
  item,
  geom,
  ...
  const glyphs = estimateGlyphsForRun(
    item.str || "",
    originalWidth,
    geom.fontSize,
  );
...
export function getOriginalBox(block = {}) {
  ...
  return {
    ...
    fontSize,
    baselineOffset:
      block.originalBaselineOffset ?? block.baselineOffset ?? fontSize * 0.8,
    lineHeight:
      block.originalLineHeight ??
      block.lineHeight ??
      block.height ??
      fontSize * 1.1,
  };
...
export function planSingleLineFit({
  ...
  const targetWidth = preserveWidth
    ? Math.max(originalBox.width / baseScale, 0.1)
    : naturalWidth;
  const chars = textChars(text);
  ...
  if (
    ...
    !targetWidth ||
    chars.length <= 1 ||
    naturalWidth <= targetWidth + tolerance
  ) {
    return {
      text,
      size,
...
export function layoutTextForBlock({
  ...
  const lineHeight = Math.max(
    (originalBox.lineHeight || originalBox.height) / baseScale,
    size * 1.05,
  );
  const maxLines = Math.max(
    1,
    Math.floor(
      Math.max(originalBox.height, originalBox.lineHeight) /
        Math.max(originalBox.lineHeight, 1),
    ),
  );
  ...
  const overflow =
    fittedLines.some((line) => line.overflow) || lines.length > maxLines;
  ...
  return {
    ...
    status: overflow
      ? "overflow"
      : fittedLines.some((line) => line.status !== "natural")
        ? "fit"
        : "natural",
  };
}
